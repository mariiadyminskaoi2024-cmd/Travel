...
const trips = [
  ...
  ['madeira','Мадейра: лавові басейни, океан і левади','Португалія','Активний острівний escape','warm','active','reset','medium',7,820,'https://images.unsplash.com/photo-1544085311-11a028465b03?auto=format&fit=crop&w=1200&q=80',['океан','треки','острів','природа'],'Funchal Garden Stay','Авіарейс до Фуншала + авто','Для тих, хто хоче теплий клімат, океан, мальовничі маршрути левадами та багато природи без хаосу великого міста.'],
  ['edinburgh','Единбург: замки, книжкові крамниці і містика','Шотландія','Атмосферний культурний city break','cold','calm','inspiration','medium',4,690,'https://images.unsplash.com/photo-1506377585622-bedcbb027afc?auto=format&fit=crop&w=1200&q=80',['замки','книги','історія','атмосфера'],'Royal Mile Cozy Hotel','Авіарейс до Единбурга','Камерна подорож для натхнення: старе місто, замок, затишні паб-крамниці та драматичні краєвиди.'],
  ['marrakech','Марракеш: ріади, спеції і пустеля','Марокко','Тепла емоційна пригода','warm','active','inspiration','medium',6,780,'https://images.unsplash.com/photo-1548018560-c7196548e84d?auto=format&fit=crop&w=1200&q=80',['ринок','пустеля','ріад','спеції'],'Medina Riad Stay','Авіарейс до Марракеша','Яскравий маршрут із базарами, садами, марокканською кухнею і коротким виїздом до пустелі.'],
  ['tokyo','Токіо: неон, храми і гастро-квести','Японія','Мегаполіс для сильних вражень','mild','active','inspiration','high',8,1850,'https://images.unsplash.com/photo-1540959733332-eab4deabeeaf?auto=format&fit=crop&w=1200&q=80',['неон','храми','їжа','технології'],'Shinjuku Smart Hotel','Авіарейс до Токіо','Для мандрівників, які хочуть контраст: храми, технології, нічне місто, суші-бари та чіткий маршрут без перевантаження.']
...
function scoreTrip(trip, answers) {
  const weights = { mood: 32, budget: 28, activity: 20, climate: 18 }
  let score = 7
  if (answers.mood === trip.mood) score += weights.mood
  else if ((answers.mood === 'reset' && trip.mood === 'relax') || (answers.mood === 'relax' && trip.mood === 'reset')) score += 14
  else if (answers.mood === 'romance' && trip.activity === 'calm') score += 8

  const diff = Math.abs((budgetRank[answers.budget] || 2) - (budgetRank[trip.budget] || 2))
  if (diff === 0) score += weights.budget
  else if (diff === 1) score += answers.budget === 'high' ? 15 : 8
  else score -= 18

  if (answers.activity === trip.activity) score += weights.activity
  else if (answers.activity === 'balanced' || trip.activity === 'balanced') score += 10

  if (answers.climate === trip.climate) score += weights.climate
  else if (answers.climate === 'mild' || trip.climate === 'mild') score += 7

  if (answers.budget === 'low' && trip.budget === 'high') score -= 12
  if (answers.activity === 'calm' && trip.activity === 'active') score -= 8
  return Math.max(18, Math.min(99, Math.round(score)))
}

function matchReasons(trip, answers) {
  const reasons = []
  if (answers.mood === trip.mood) reasons.push(`настрій: ${moodLabels[answers.mood]}`)
  if (answers.budget === trip.budget) reasons.push(`бюджет: ${budgetLabels[answers.budget]}`)
  if (answers.activity === trip.activity) reasons.push(`темп: ${answers.activity === 'calm' ? 'спокійний' : answers.activity === 'active' ? 'активний' : 'збалансований'}`)
  if (answers.climate === trip.climate) reasons.push(`клімат: ${answers.climate === 'warm' ? 'теплий' : answers.climate === 'cold' ? 'прохолодний' : 'помірний'}`)
  if (!reasons.length) reasons.push('частковий збіг за стилем подорожі')
...
function Header({ page, setPage, user, onAuth, onLogout }) {
  ...
  const links = [['home','Головна'],['quiz','Smart Quiz'],['recommendations','Ідеї'],['profile','Профіль']]
...
function Home({ setPage, user }) {
  return <main><section className="hero"><div><Badge><Sparkles size={14}/> AI Smart Travel Planner</Badge><h1>Подорож, яка підлаштовується під твій настрій, бюджет і стиль.</h1><p>Оновлена версія має вхід/вихід з акаунта, збереження обраного, історію тестів, чернетки бронювань, точніший підбір за тестом і значно більшу базу ідей подорожей.</p><div className="row"><Button onClick={()=>setPage('quiz')}><Sparkles size={18}/> Пройти Smart Quiz</Button><Button variant="ghost" onClick={()=>setPage('recommendations')}><Compass size={18}/> Дивитись ідеї</Button></div><div className="stats">{[[trips.length,'ідей подорожей'],[user?'online':'demo','акаунт'],[firebaseEnabled?'Firebase':'local','backend mode']].map(([n,t])=><div key={t}><b>{n}</b><span>{t}</span></div>)}</div></div><div className="hero-img"><img src="https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&fit=crop&w=1400&q=80"/><div><b>Персональний підбір</b><span>Smart Quiz → Matchmaker → Daily Planner</span></div></div></section><section className="features">{[[LogIn,'Auth','Вхід, реєстрація та вихід з акаунта.'],[Heart,'Профіль мандрівника','Обране, історія тестів і чернетки бронювань.'],[Compass,'Більше напрямків',`${trips.length} travel-ідей з фільтрами.`],[Sparkles,'Surprise trip','Кнопка “Здивуй мене” обирає сильний збіг.']].map(([Icon,t,d])=><div className="feature" key={t}><Icon/><h3>{t}</h3><p>{d}</p></div>)}</section></main>
}
...
function Recommendations({ answers, favorites, onFavorite, setSelected, setPage }) {
  ...
  const ranked=useMemo(()=>trips.map(t=>({...t,score:scoreTrip(t,answers),reasons:matchReasons(t,answers)})).filter(t=>(budget==='all'||t.budget===budget) && `${t.title} ${t.country} ${t.tags.join(' ')}`.toLowerCase().includes(query.toLowerCase())).sort((a,b)=> sort==='price'?a.baseCost-b.baseCost:sort==='days'?a.days-b.days:b.score-a.score),[answers,query,budget,sort])
  const top = ranked[0]
  function surprise(){ const pool = ranked.filter(t=>t.score>=70); const list = pool.length ? pool : ranked; const pick = list[Math.floor(Math.random()*list.length)]; if(pick){setSelected(pick); setPage('details')} }
  return <main className="page"><div className="section-head"><div><Badge><Sparkles size={14}/> AI Matchmaker</Badge><h2>Ідеї подорожей</h2><p>Підбір тепер жорсткіше враховує бюджет, настрій, темп і клімат. Найкращі збіги піднімаються вгору, а невідповідні варіанти отримують нижчий Match Score.</p></div><Button onClick={surprise}><Sparkles size={18}/> Здивуй мене</Button></div>
  <div className="match-panel"><div><small>Твій travel-профіль</small><b>{moodLabels[answers.mood]} · {budgetLabels[answers.budget]}</b><span>{answers.activity === 'calm' ? 'спокійний темп' : answers.activity === 'active' ? 'активний темп' : 'збалансований темп'} · {answers.climate === 'warm' ? 'теплий клімат' : answers.climate === 'cold' ? 'прохолодний клімат' : 'помірний клімат'}</span></div>{top&&<div><small>Найкращий збіг</small><b>{top.title}</b><span>{top.score}% · {top.reasons.join(' · ')}</span></div>}</div>
  <div className="filters"><label><Search size={16}/><input placeholder="Пошук: море, гори, Прага..." value={query} onChange={e=>setQuery(e.target.value)}/></label><select value={budget} onChange={e=>setBudget(e.target.value)}><option value="all">Будь-який бюджет</option><option value="low">До €400</option><option value="medium">€400–800</option><option value="high">€800+</option></select><select value={sort} onChange={e=>setSort(e.target.value)}><option value="score">За Match Score</option><option value="price">За ціною</option><option value="days">За днями</option></select></div><div className="trip-grid">{ranked.map(trip=><article className="trip-card" key={trip.id}><div className="trip-img"><img src={trip.image}/><span>{trip.score}% Match</span><button onClick={()=>onFavorite(trip.id)}><Heart className={favorites.includes(trip.id)?'liked':''}/></button></div><div className="trip-body"><div className="tags">{trip.tags.slice(0,3).map(t=><Badge key={t}>{t}</Badge>)}</div><h3>{trip.title}</h3><p>{trip.description}</p><div className="reasons">{trip.reasons.map(r=><em key={r}>{r}</em>)}</div><div className="meta"><span><Wallet/>€{trip.baseCost}</span><span><CalendarDays/>{trip.days} днів</span><span><MapPin/>{trip.country}</span></div><Button onClick={()=>{setSelected(trip);setPage('details')}}>Деталі</Button></div></article>)}</div></main>
}

function Details({ trip, answers, favorites, onFavorite, setPage, user, setUserData }) {
  const selected=trip||trips[0]; const currentScore = selected.score || scoreTrip(selected, answers); const currentReasons = selected.reasons || matchReasons(selected, answers); const budgetRows=[['Квитки',.34],['Житло',.38],['Їжа',.16],['Активності',.12]]
...
function Chat() { const [open,setOpen]=useState(false); const [text,setText]=useState(''); const [msgs,setMsgs]=useState([{role:'bot',text:'Привіт! Я Travel Assistant. Можу порадити напрямок, пояснити бюджет або допомогти з тестом.'}]); function send(){ if(!text.trim())return; const t=text; setMsgs(m=>[...m,{role:'user',text:t}]); setText(''); setTimeout(()=>setMsgs(m=>[...m,{role:'bot',text:t.toLowerCase().includes('бюдж')?'Бюджет сильно впливає на Match Score: дорогі поїздки не будуть високо показуватись для бюджету до €400.':'Спробуй Smart Quiz або кнопку “Здивуй мене” — вона відкриє випадковий напрямок із високим збігом.'}]),250)} return <><button className="chat-fab" onClick={()=>setOpen(true)}><MessageCircle/></button><AnimatePresence>{open&&<motion.div className="chat" initial={{opacity:0,y:25}} animate={{opacity:1,y:0}} exit={{opacity:0,y:25}}><div className="chat-head"><b>Travel Assistant</b><button onClick={()=>setOpen(false)}><X size={18}/></button></div><div className="chat-body">{msgs.map((m,i)=><div key={i} className={m.role}>{m.text}</div>)}</div><div className="chat-input"><input value={text} onChange={e=>setText(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()} placeholder="Напиши питання..."/><button onClick={send}><Send size={18}/></button></div></motion.div>}</AnimatePresence></> }
...
export default function App(){
  ...
  return <div><Header page={page} setPage={setPage} user={user} onAuth={()=>setAuthOpen(true)} onLogout={onLogout}/>{page==='home'&&<Home setPage={setPage} user={user}/>} {page==='quiz'&&<Quiz answers={answers} setAnswers={setAnswers} setPage={setPage} user={user} setUserData={setData}/>} {page==='recommendations'&&<Recommendations answers={answers} favorites={data.favorites||[]} onFavorite={onFavorite} setSelected={setSelected} setPage={setPage}/>} {page==='details'&&<Details trip={selected} answers={answers} favorites={data.favorites||[]} onFavorite={onFavorite} setPage={setPage} user={user} setUserData={setData}/>} {page==='profile'&&<Profile user={user} data={data} setPage={setPage} setSelected={setSelected}/>}<footer><b>MapMyMood v3</b><p>React + Vite · Auth · Favorites · History · Booking drafts · точний Matchmaker · GitHub Pages base /Travel/</p></footer><AuthModal open={authOpen} onClose={()=>setAuthOpen(false)} onAuthSuccess={async u=>{setUser(u);setData(await loadUserData(u))}}/><Chat/></div>
}
